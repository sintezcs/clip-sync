...
Decoded UTF-8 text is capped at 1 MiB. Encoded image content is capped at 50 MiB; supported image bytes and declared MIME must agree and decode successfully. Width and height are each 1–8192 pixels, with at most 24,000,000 pixels. The JSON envelope cap is `((50 * 1024 * 1024 + 2) / 3) * 4 + 4096` bytes on the Mac. Timestamp arithmetic is checked without signed subtraction overflow. Local Mac TIFF conversion produces bounded PNG; TIFF is not a wire MIME. On Android 9+, automatic clipboard HEIC/HEIF input is decoded locally with orientation applied and converted to bounded PNG; HEIC/HEIF are not wire MIME types. The same 50 MiB input/output, 8192-pixel side and 24-megapixel limits apply. Images are applied to the clipboard, not saved as received files in Documents.
